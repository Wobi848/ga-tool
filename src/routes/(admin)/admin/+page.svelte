			...
			<div class="header-actions">
				...
				>
					<button
						...
						title="Test-E-Mail an eigene Adresse senden">📧 Test-Mail</button
					>
				</form>
				<button
					type="button"
		...
		{#if showCreateForm}
			...
			>
				...
				<button
					type="submit"
					class="btn btn--primary btn--sm"
					disabled={!createEmail || createPw.length < 8}>Anlegen</button
				>
			</form>
		{/if}
	</div>
			...
			<tbody>
				{#each filteredUsers as u (u.id)}
					<tr class:row--banned={u.banned}>
							...
							<div class="user-cell">
								...
								<div>
									...
									{#if u.company || u.profileRole}
										<div class="user-meta">
											{[u.profileRole, u.company].filter(Boolean).join(' · ')}
										</div>
									{/if}
								</div>
							</div>
						...
						<td class="date-cell" title={u.lastLogin ? formatDate(u.lastLogin) : ''}
							>{formatRelative(u.lastLogin)}</td
						>
						<td class="fav-cell">
								...
								<div class="fav-pills">
									{#each u.favorites.slice(0, 4) as f}
										<span
											...
											style:color={typeColor[f.type] ?? '#64748b'}
											title={f.title}
										>
				...
				{#if filteredUsers.length === 0}
					<tr
						><td
							colspan="7"
							style="text-align:center;padding:2rem;color:var(--muted);font-size:0.875rem;"
							>Keine Benutzer gefunden</td
						></tr
					>
				{/if}
			</tbody>
...
{#snippet Trash()}
	<svg
		...
		stroke-linecap="round"
	>
		<polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14H6L5 6" /><path d="M10 11v6" /><path
			d="M14 11v6"
		/><path d="M9 6V4h6v2" />
	</svg>
{/snippet}
