<script lang="ts">
	...
	const slugsByModule = $derived(
		data.topSlugs.reduce(
			(acc, r) => {
				const mod = r.module;
				if (!acc[mod]) acc[mod] = [];
				...
				<div class="bar-chart bar-chart--sm">
					{#each data.regDaily as d (d)}
						<div class="bar-col" title="{fmtDay(d.day)}: {d.cnt} Registrierungen">
							<div
								class="bar"
								style:height="{maxReg > 0 ? (d.cnt / maxReg) * 100 : 0}%"
								style:background={d.cnt > 0 ? '#0d9488' : 'var(--border)'}
							></div>
						</div>
					{/each}
				</div>
			...
			<div class="fav-list">
				{#each data.topFavorites as f (f)}
					<div class="fav-row">
						<span
							class="fav-badge"
							style:background="color-mix(in srgb, {favTypeColor[f.type] ?? '#64748b'} 15%, transparent)"
							style:color={favTypeColor[f.type] ?? '#64748b'}>{f.type}</span
						>
						...
						<div class="fav-bar-wrap">
							<div
								...
								style:background={favTypeColor[f.type] ?? '#64748b'}
							></div>
						</div>
...
<style>
	...
	.user-row {
		display: grid;
		grid-template-columns: 1.25rem 1fr 5rem 2.5rem;
		align-items: center;
		gap: 0.5rem;
		font-size: 0.8125rem;
	}
	...
	.fav-row {
		display: grid;
		grid-template-columns: 5.5rem 1fr 10rem 2.5rem;
		align-items: center;
		gap: 0.5rem;
		font-size: 0.8125rem;
	}
