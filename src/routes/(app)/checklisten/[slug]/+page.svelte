<script lang="ts">
	...
	onMount(() => {
		...
		status = s.status;
		notes = s.notes;
		context = { ort: '', anlage: '', techniker: '', datum: new Date().toISOString().slice(0, 10), ...s.context };
	});
	...
	const doneCritical = $derived(
		template.sections
			.flatMap((s) => s.items)
			.filter((i) => i.critical && status[i.id]).length
	);
...
<style>
	...
	.area-chip { background: color-mix(in srgb, var(--color-secondary) 15%, transparent); color: var(--color-secondary); }
	.norm-chip { background: var(--surface-hover); color: var(--muted); font-family: ui-monospace, monospace; }
	.description { font-size: 0.875rem; color: var(--text); line-height: 1.5; margin: 0.75rem 0 0; }
	...
	@media (max-width: 480px) { .context-box { grid-template-columns: 1fr; } }
	.ctx-field { display: flex; flex-direction: column; gap: 0.2rem; }
	.ctx-field span { font-size: 0.7rem; color: var(--muted); font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
	.ctx-field input { background: var(--bg); border: 1px solid var(--border); border-radius: 0.4rem; padding: 0.35rem 0.5rem; font-size: 0.85rem; color: var(--text); font-family: inherit; }
	.ctx-field input:focus { outline: none; border-color: var(--color-primary); }

	.progress-section { margin: 1rem 0 1.25rem; }
	.progress-stats { display: flex; align-items: baseline; gap: 0.75rem; margin-bottom: 0.4rem; flex-wrap: wrap; }
	.progress-pct { font-size: 1.5rem; font-weight: 700; font-variant-numeric: tabular-nums; }
	.progress-counts { font-size: 0.8125rem; color: var(--muted); }
	.critical-ok { color: #16a34a; }
	.critical-warn { color: #dc2626; }
	.saved-flash { font-size: 0.7rem; color: #16a34a; margin-left: auto; animation: fadeOut 1.2s; }
	@keyframes fadeOut { 0% { opacity: 1; } 100% { opacity: 0; } }
	.progress-bar { height: 8px; background: var(--border); border-radius: 4px; overflow: hidden; }
	.progress-fill { height: 100%; transition: width 0.3s; }

	.section { background: var(--surface); border: 1px solid var(--border); border-radius: 0.75rem; padding: 0.75rem 1rem; margin-bottom: 0.5rem; }
	.section-header { display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 0.4rem; }
	.section-header h2 { font-size: 0.85rem; font-weight: 600; color: var(--color-primary); margin: 0; text-transform: uppercase; letter-spacing: 0.04em; }
	.section-count { font-size: 0.7rem; color: var(--muted); font-variant-numeric: tabular-nums; }

	.items { list-style: none; padding: 0; margin: 0; }
	.item { padding: 0.45rem 0; border-bottom: 1px solid var(--border); position: relative; }
	.item:last-child { border-bottom: none; }
	.item.done .item-title { color: var(--muted); text-decoration: line-through; opacity: 0.75; }

	.item-main { display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding-right: 4rem; }
	.item-main input[type='checkbox'] { width: 1.05rem; height: 1.05rem; accent-color: var(--color-primary); cursor: pointer; flex-shrink: 0; }
	.item-title { font-size: 0.875rem; color: var(--text); line-height: 1.4; }
	.critical-badge { display: inline-block; width: 1rem; height: 1rem; line-height: 1rem; text-align: center; font-size: 0.7rem; font-weight: 700; background: #dc2626; color: white; border-radius: 50%; flex-shrink: 0; }
	.norm-tag { font-size: 0.65rem; padding: 0.1rem 0.35rem; border-radius: 0.25rem; background: var(--surface-hover); color: var(--muted); font-family: ui-monospace, monospace; }

	.item-actions { position: absolute; right: 0; top: 0.4rem; display: flex; gap: 0.25rem; }
	.action-btn { display: flex; align-items: center; justify-content: center; width: 1.6rem; height: 1.6rem; background: transparent; border: 1px solid var(--border); border-radius: 0.3rem; color: var(--muted); cursor: pointer; }
	.action-btn:hover { color: var(--color-primary); border-color: var(--color-primary); }
	.action-btn.active { color: var(--color-primary); border-color: var(--color-primary); background: color-mix(in srgb, var(--color-primary) 10%, transparent); }

	.item-hint { font-size: 0.75rem; color: var(--muted); background: var(--bg); border-left: 2px solid var(--color-primary); padding: 0.4rem 0.6rem; margin: 0.4rem 0 0 1.5rem; border-radius: 0 0.3rem 0.3rem 0; line-height: 1.4; }
	.item-note { display: block; width: calc(100% - 1.5rem); margin: 0.4rem 0 0 1.5rem; background: var(--bg); border: 1px solid var(--border); border-radius: 0.3rem; padding: 0.4rem 0.5rem; font-size: 0.8125rem; color: var(--text); font-family: inherit; resize: vertical; }
	.item-note:focus { outline: none; border-color: var(--color-primary); }

	.done-banner { background: color-mix(in srgb, #16a34a 12%, transparent); border: 1px solid #16a34a; border-radius: 0.75rem; padding: 0.75rem 1rem; margin: 1rem 0; }
	.done-banner strong { display: block; color: #16a34a; margin-bottom: 0.25rem; }
	.done-banner p { font-size: 0.8125rem; color: var(--text); margin: 0; }

	.action-bar { display: flex; gap: 0.5rem; justify-content: flex-end; margin-top: 1rem; padding-top: 0.75rem; border-top: 1px solid var(--border); }
	.btn-primary, .btn-secondary { display: inline-flex; align-items: center; gap: 0.4rem; font-size: 0.875rem; font-weight: 500; padding: 0.5rem 1rem; border-radius: 0.5rem; cursor: pointer; font-family: inherit; }
