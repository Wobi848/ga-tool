...
<div class="calc-page">
	<header class="calc-header">
		...
		<div class="calc-title-row">
			...
			<FavButton
				type="rechner"
				slug="leitungslaenge"
				title={$_('rechner.leitungslaenge.name')}
				size={20}
			/>
		</div>
	</header>
	...
	<div class="calc-section">
		...
		<div class="calc-field" style="border-top:none">
			<span class="calc-field-label"
				>{$_('rechner.leitungslaengeUi.devicePreset')}
		...
		<div class="calc-field">
			<label class="calc-field-label" for="u-source"
				>{$_('rechner.leitungslaengeUi.sourceVoltage')}</label
			>
		...
		<div class="calc-field">
			<label class="calc-field-label" for="u-min"
				>{$_('rechner.leitungslaengeUi.minVoltageDevice')}</label
			>
			<div class="calc-input-wrap">
				<input
					id="u-min"
		...
		<div class="calc-field">
			...
			<div class="seg-wrap">
				<button
					type="button"
					class="seg-btn"
					class:seg-active={!useWatt}
					onclick={() => (useWatt = false)}>{$_('rechner.leitungslaengeUi.currentA')}</button
				>
				<button
					type="button"
					class="seg-btn"
					class:seg-active={useWatt}
					onclick={() => (useWatt = true)}>{$_('rechner.leitungslaengeUi.powerW')}</button
				>
			</div>
		</div>
		...
		{#if useWatt}
			<div class="calc-field">
				<label class="calc-field-label" for="pow-w"
					>{$_('rechner.leitungslaengeUi.powerPerDevice')}</label
				>
				<div class="calc-input-wrap">
		...
		{:else}
			<div class="calc-field">
				<label class="calc-field-label" for="curr"
					>{$_('rechner.leitungslaengeUi.currentPerDevice')}</label
				>
				<div class="calc-input-wrap">
					<input
						id="curr"
		...
		<div class="calc-field">
			<label class="calc-field-label" for="dev-count"
				>{$_('rechner.leitungslaengeUi.deviceCount')}
				<span class="calc-field-hint"
					>{$_('rechner.leitungslaengeUi.totalCurrentHint', { values: { i: fmt(I, 3) } })}</span
				>
			</label>
			<div class="calc-input-wrap">
				<input
		...
		<div class="calc-field">
			<span class="calc-field-label"
				>{$_('rechner.leitungslaengeUi.devicePlacement')}
				<span class="calc-field-hint"
					>{placement === 'end'
						? $_('rechner.leitungslaengeUi.placementEndHint')
						: $_('rechner.leitungslaengeUi.placementDistHint')}</span
				>
			</span>
			<div class="seg-wrap">
				<button
					type="button"
					class="seg-btn"
					class:seg-active={placement === 'end'}
					onclick={() => (placement = 'end')}>{$_('rechner.leitungslaengeUi.atEnd')}</button
				>
				<button
					type="button"
					class="seg-btn"
					class:seg-active={placement === 'dist'}
					onclick={() => (placement = 'dist')}>{$_('rechner.leitungslaengeUi.distributed')}</button
				>
			</div>
		</div>
		...
		{#if mode !== 'querschnitt'}
			<div class="calc-field">
				<label class="calc-field-label" for="sect"
					>{$_('rechner.leitungslaengeUi.crossSection')}</label
				>
				<div class="calc-input-wrap">
	...
	{#if mode === 'spannungsfall'}
		<div class="calc-result-section">
			<div class="calc-result">
				...
				<span class="calc-result-value primary"
					>{fmt(deltaU, 3)}<span class="calc-result-unit">V</span></span
				>
			</div>
			<div class="calc-result">
				...
				<span
					...
					style="color:{voltOk ? 'var(--color-ok,#16a34a)' : '#dc2626'}"
					>{fmt(uEnd, 3)}<span class="calc-result-unit">V</span></span
				>
			</div>
			<div class="calc-result">
				<span class="calc-result-label">{$_('rechner.leitungslaengeUi.voltDropPct')}</span>
				<span class="calc-result-value"
					>{fmt(dropPct, 2)}<span class="calc-result-unit">%</span></span
				>
			</div>
			<div class="calc-result">
				<span class="calc-result-label">{$_('rechner.leitungslaengeUi.cableResistance')}</span>
				<span class="calc-result-value"
					>{fmt(rTotal, 3)}<span class="calc-result-unit">Ω</span></span
				>
			</div>
		</div>
		...
		{#if !voltOk}
			<div class="calc-warning">
				⚠ {$_('rechner.leitungslaengeUi.warnVoltageLow', {
					values: { u: fmt(uEnd, 2), umin: uMinDevice }
				})}
			</div>
		{/if}
		{#if !currentOk}
			<div class="calc-warning">
				⚠ {$_('rechner.leitungslaengeUi.warnCurrentHigh', {
					values: { i: fmt(I, 2), imax: ampacity[crossSection], cs: crossSection }
				})}
			</div>
		{/if}
		<p class="calc-info">
			{$_('rechner.leitungslaengeUi.infoCalc', {
				values: {
					rho: RHO_CU,
					l: length * 2,
					i: fmt(Ieff, 3),
					placement:
						placement === 'end'
							? $_('rechner.leitungslaengeUi.placementEndLabel')
							: $_('rechner.leitungslaengeUi.placementDistLabel')
				}
			})}
		</p>
	{/if}
		...
		<div class="calc-result-section">
			<div class="calc-result">
				<span class="calc-result-label"
					>{$_('rechner.leitungslaengeUi.maxCableLength', { values: { cs: crossSection } })}</span
				>
				<span class="calc-result-value primary"
					>{fmt(maxLength, 1)}<span class="calc-result-unit">m</span></span
				>
			</div>
			<div class="calc-result">
				<span class="calc-result-label">{$_('rechner.leitungslaengeUi.allowedDrop')}</span>
				<span class="calc-result-value"
					>{fmt(uDropMax, 2)}<span class="calc-result-unit"
						>V ({fmt((uDropMax / uSource) * 100, 1)} %)</span
					></span
				>
			</div>
		</div>
			...
			{#each stdSections as s, idx (idx)}
				...
				<div class="calc-field" style={idx === 0 ? 'border-top:none' : ''}>
					<span class="calc-field-label" class:muted={!iOk}>
						{s} mm²
						{#if !iOk}<span class="calc-field-hint"
								>⚠ {$_('rechner.leitungslaengeUi.currentTooHigh', {
									values: { imax: ampacity[s] }
								})}</span
							>{/if}
					</span>
	...
	{#if mode === 'querschnitt'}
		<div class="calc-result-section">
			<div class="calc-result">
				...
				<span class="calc-result-value"
					>{fmt(reqSection, 3)}<span class="calc-result-unit">mm²</span></span
				>
			</div>
			<div class="calc-result">
				<span class="calc-result-label">{$_('rechner.leitungslaengeUi.recommendedSection')}</span>
				<span class="calc-result-value primary"
					>{recSection}<span class="calc-result-unit">mm²</span></span
				>
			</div>
		</div>
		<p class="calc-info">
			{$_('rechner.leitungslaengeUi.stdSections')}
			{stdSections.join(' · ')} mm²
		</p>
	{/if}
</div>
