<script lang="ts">
	...
	let statusClass = $derived(
		sim.display.status === 'settled' ? 'ok' : sim.display.status === 'saturated' ? 'warn' : 'bad'
	);
	...
	<header class="pid-header">
		...
		<div class="header-row">
			...
			<FavButton
				type="rechner"
				slug="pid-simulator"
				title={$_('rechner.pidSimulator.name')}
				size={20}
			/>
		</div>
			...
			<div class="section">
				...
				<div class="slider-row">
					...
					<div class="slider-foot">
						...
						<span class="hint"
							>{sim.tn === 0
								? $_('rechner.pidSimulatorUi.iTermOff')
								: `≈ ${fmt(sim.tn / 60, 1)} min`}</span
						>
						<span>3600s</span>
					</div>
				</div>
				...
				<div class="slider-row">
					...
					<div class="slider-foot">
						...
						<span class="hint"
							>{sim.tv === 0
								? $_('rechner.pidSimulatorUi.dOff')
								: $_('rechner.pidSimulatorUi.dOn')}</span
						>
						<span>600s</span>
					</div>
				</div>
					...
					<div class="slider-head">
						...
						<div class="numwrap">
							<input
								...
								value={sim.winSec}
								oninput={(e) =>
									(sim.winSec = cl(parseFloat(e.currentTarget.value) || 30, 30, 1800))}
								class="numbox"
							/>
							<span class="unit">s</span>
						</div>
			...
			<section class="chart-pane">
				...
				<div class="chart-wrap">
					<div class="chart-legend">
						...
						<span class="legend-item" style="color:#4ade80">
							...
							{$_('rechner.pidSimulatorUi.actualValueX')} &nbsp;<b
								>{fmt(sim.display.pv, 2)} {sim.unit}</b
							>
						</span>
				...
				<div class="readouts">
					<div class="readout">
						<div class="ro-label" style="color:#38bdf8">
							<span class="ro-swatch" style="background:#38bdf8"></span>{$_(
								'rechner.pidSimulatorUi.setpointW'
							)}
						</div>
					...
					<div class="readout">
						<div class="ro-label" style="color:#4ade80">
							<span class="ro-swatch" style="background:#4ade80"></span>{$_(
								'rechner.pidSimulatorUi.actualValueX'
							)}
						</div>
						<div class="ro-value">
							{fmt(sim.display.pv, 2)}<span class="ro-unit">{sim.unit}</span>
						</div>
					</div>
					<div class="readout">
						<div class="ro-label" style="color:#fb923c">
							<span class="ro-swatch" style="background:#fb923c"></span>{$_(
								'rechner.pidSimulatorUi.outputY'
							)}
						</div>
					...
					<div class="readout">
						<div class="ro-label" style="color:#f87171">
							<span class="ro-swatch" style="background:#f87171"></span>{$_(
								'rechner.pidSimulatorUi.controlDeviation'
							)}
						</div>
						<div class="ro-value">
							{fmt(sim.display.e, 2)}<span class="ro-unit">{sim.unit}</span>
						</div>
					</div>
					<div class="readout">
						...
						<div
							class="ro-value"
							style:color={sim.display.dist === 0 && sim.display.autoDist === 0
								? 'var(--muted)'
								: 'var(--text)'}
						>
							{sim.display.dist + sim.display.autoDist >= 0 ? '+' : ''}{fmt(
								sim.display.dist + sim.display.autoDist,
								2
							)}<span class="ro-unit">{sim.unit}</span>
						</div>
						{#if sim.autoDistActive}
							<div class="ro-sub">
								...
								<span style="color:#f87171"
									>{sim.display.autoDist >= 0 ? '+' : ''}{fmt(sim.display.autoDist, 2)}</span
								>
							</div>
						{/if}
					</div>
				...
				<div class="status-strip">
					...
					<div class="strip-cell grow">
						<span class="term" style="color:#4ade80"
							>P <b>{fmt(sim.display.p, 2)}</b><em>%</em></span
						>
						<span class="term" style="color:#38bdf8"
							>I <b>{fmt(sim.display.i, 2)}</b><em>%</em></span
						>
						<span class="term" style="color:#fb923c"
							>D <b>{fmt(sim.display.d, 2)}</b><em>%</em></span
						>
						<span class="term"
							>Σ <b>{fmt(sim.display.p + sim.display.i + sim.display.d, 2)}</b><em>%</em></span
						>
					</div>
			...
			<div class="env-panel">
				...
				<div class="env-section">
						...
						<div class="slider-head">
							...
							<div class="numwrap">
								<input
									...
									value={sim.distStep}
									oninput={(e) =>
										(sim.distStep = cl(parseFloat(e.currentTarget.value) || 0, -50, 50))}
									class="numbox"
								/><span class="unit">{sim.unit}</span>
							</div>
						</div>
					...
					{#if sim.display.dist !== 0}
						...
						>
							{$_('rechner.pidSimulatorUi.resetDist', {
								values: { val: fmt(sim.display.dist, 1), unit: sim.unit }
							})}
						</button>
					{/if}
				</div>
				...
				<div class="env-section">
					...
					<div class="autodist-header">
						<label class="toggle-wrap">
							...
							<span class="toggle-label"
								>{sim.autoDistActive
									? $_('rechner.pidSimulatorUi.active')
									: $_('rechner.pidSimulatorUi.inactive')}</span
							>
						</label>
						{#if sim.autoDistActive}
							<span class="autodist-live" style="color:#f87171">
								d̃ = {fmt(sim.display.autoDist, 2)}
								{sim.unit}
							</span>
						{/if}
					</div>
					{#if sim.autoDistActive}
						...
						<div class="slider-row">
							<div class="slider-head">
								<div class="slider-label">
									<span class="key" style="color:#f87171">A</span><span class="name"
										>{$_('rechner.pidSimulatorUi.amplitude')}</span
									>
								</div>
								<div class="numwrap">
									<input
										...
										max="50"
										step="0.5"
										value={sim.autoDistAmp}
										oninput={(e) =>
											(sim.autoDistAmp = cl(parseFloat(e.currentTarget.value) || 0.1, 0.1, 50))}
										class="numbox"
									/><span class="unit">{sim.unit}</span>
								</div>
							</div>
						...
						{#if sim.autoDistType !== 'noise'}
							<div class="slider-row">
								<div class="slider-head">
									<div class="slider-label">
										<span class="key" style="color:#f87171">T</span><span class="name"
											>{$_('rechner.pidSimulatorUi.period')}</span
										>
									</div>
									<div class="numwrap">
										<input
											...
											max="600"
											step="5"
											value={sim.autoDistPeriod}
											oninput={(e) =>
												(sim.autoDistPeriod = cl(parseFloat(e.currentTarget.value) || 5, 5, 600))}
											class="numbox"
										/><span class="unit">s</span>
									</div>
								</div>
								...
								/>
								<div class="slider-foot">
									<span>5s</span><span class="hint">≈ {fmt(sim.autoDistPeriod / 60, 1)} min</span
									><span>600s</span>
								</div>
							</div>
						{/if}
						<div class="slider-row">
							<div class="slider-head">
								<div class="slider-label">
									<span class="key" style="color:#a78bfa">Tc</span><span class="name"
										>{$_('rechner.pidSimulatorUi.filterPT1')}</span
									>
								</div>
								<div class="numwrap">
									<input
										...
										max="120"
										step="1"
										value={sim.autoDistTc}
										oninput={(e) =>
											(sim.autoDistTc = cl(parseFloat(e.currentTarget.value) || 0, 0, 120))}
										class="numbox"
									/><span class="unit">s</span>
								</div>
							</div>
							...
							/>
							<div class="slider-foot">
								<span>0s</span><span class="hint"
									>{sim.autoDistTc === 0
										? $_('rechner.pidSimulatorUi.noFilter')
										: `PT1 ≈ ${fmt(sim.autoDistTc, 0)}s`}</span
								><span>120s</span>
							</div>
						</div>
						<div class="slider-row">
							<div class="slider-head">
								<div class="slider-label">
									<span class="key" style="color:#a78bfa">B</span><span class="name"
										>{$_('rechner.pidSimulatorUi.bias')}</span
									>
								</div>
								<div class="numwrap">
									<input
										...
										max="20"
										step="0.5"
										value={sim.autoDistBias}
										oninput={(e) =>
											(sim.autoDistBias = cl(parseFloat(e.currentTarget.value) || 0, -20, 20))}
										class="numbox"
									/><span class="unit">{sim.unit}</span>
								</div>
							</div>
							...
							/>
							<div class="slider-foot">
								<span>−20</span><span class="hint"
									>{sim.autoDistBias === 0
										? $_('rechner.pidSimulatorUi.noOffset')
										: `${sim.autoDistBias > 0 ? '+' : ''}${fmt(sim.autoDistBias, 1)} ${sim.unit}`}</span
								><span>+20</span>
							</div>
						</div>
					{/if}
				...
				<div class="env-section">
					...
					{#if sim.spAutoActive}
						<div class="slider-row" style="margin-top:8px">
							<div class="slider-head">
								<div class="slider-label">
									<span class="key" style="color:#38bdf8">W↑</span><span class="name"
										>{$_('rechner.pidSimulatorUi.dayValue')}</span
									>
								</div>
								<div class="numwrap">
									<input
										...
										max="200"
										step="0.5"
										value={sim.spAutoHigh}
										oninput={(e) =>
											(sim.spAutoHigh = cl(parseFloat(e.currentTarget.value) || 0, 0, 200))}
										class="numbox"
									/><span class="unit">{sim.unit}</span>
								</div>
							</div>
						...
						<div class="slider-row">
							<div class="slider-head">
								<div class="slider-label">
									<span class="key" style="color:#7dd3fc">W↓</span><span class="name"
										>{$_('rechner.pidSimulatorUi.nightValue')}</span
									>
								</div>
								<div class="numwrap">
									<input
										...
										value={sim.spAutoLow}
										oninput={(e) =>
											(sim.spAutoLow = cl(parseFloat(e.currentTarget.value) || 0, 0, 200))}
										class="numbox"
									/><span class="unit">{sim.unit}</span>
								</div>
							</div>
							<input
								...
								class="slider"
								style="accent-color:#7dd3fc"
							/>
							<div class="slider-foot">
								<span></span><span class="hint"
									>Δ = {fmt(Math.abs(sim.spAutoHigh - sim.spAutoLow), 1)} {sim.unit}</span
								><span></span>
							</div>
						</div>
						<div class="slider-row">
							<div class="slider-head">
								<div class="slider-label">
									<span class="key" style="color:#38bdf8">T</span><span class="name"
										>{$_('rechner.pidSimulatorUi.switchPeriod')}</span
									>
								</div>
								<div class="numwrap">
									<input
										...
										max="1800"
										step="10"
										value={sim.spAutoPeriod}
										oninput={(e) =>
											(sim.spAutoPeriod = cl(parseFloat(e.currentTarget.value) || 10, 10, 1800))}
										class="numbox"
									/><span class="unit">s</span>
								</div>
							</div>
							...
							/>
							<div class="slider-foot">
								<span>10s</span><span class="hint">≈ {fmt(sim.spAutoPeriod / 60, 1)} min/Phase</span
								><span>1800s</span>
							</div>
						</div>
					{/if}
