<script>
  ...
  function draw() {
    ...
    for (const p of s.history) {
      ...
      hasData = true;
      if (p.sp < yMin) yMin = p.sp;  if (p.sp > yMax) yMax = p.sp;
      if (p.pv < yMin) yMin = p.pv;  if (p.pv > yMax) yMax = p.pv;
      if (p.u  < yMin) yMin = p.u;   if (p.u  > yMax) yMax = p.u;
    }
    if (!hasData) {
    ...
    yTicks.forEach(yt => ctx.fillText(fmt(yt, Math.abs(yt) >= 100 ? 0 : 1), pad.left - 8, yFor(yt)));
    ...
    if (band) ctx.fillRect(xFor(band.start), pad.top, Math.max(1, xFor(band.end) - xFor(band.start)), ch);
    ...
    if (hasData) {
      ...
      const x = xFor(last.t);
      [['pv', COLORS.pv], ['sp', COLORS.sp], ['u', COLORS.y]].forEach(([k, c]) => {
        const y = yFor(last[k]);
        ctx.fillStyle = c;
