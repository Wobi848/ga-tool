...
export class PIDSim {
  ...
  // ── Display (gedrosselt) ────────────────────────────────────────
  display = $state({
    pv: 0, y: 0, e: 0, p: 0, i: 0, d: 0, t: 0, sp: 21,
    status: 'init', sat: false, dist: 0, autoDist: 0,
  });

  // ── interne Sim (nicht reaktiv) ─────────────────────────────────
  sim = {
    t: 0, y: 0, integral: 0, prevError: 0,
    deadBuf: [], dist: 0,
    pTerm: 0, iTerm: 0, dTerm: 0, u: 0,
    history: [],
    yAmb: 0, autoDistVal: 0, autoDistFiltered: 0, spEff: 21,
  };
  ...
  reset(initY = 0) {
    this.stop();
    this.sim = {
      t: 0, y: initY, integral: 0, prevError: 0,
      deadBuf: [], dist: 0,
      pTerm: 0, iTerm: 0, dTerm: 0, u: 0,
      history: [],
      yAmb: initY, autoDistVal: 0, autoDistFiltered: 0, spEff: this.sp,
    };
    this.display = {
      pv: initY, y: 0, e: 0, p: 0, i: 0, d: 0, t: 0, sp: this.sp,
      status: 'init', sat: false, dist: 0, autoDist: 0,
    };
    if (this.running) this.start();
  }

  applyDisturbance(delta) { this.sim.dist += delta; }
  clearDisturbance()      { this.sim.dist = 0; }

  /** HVAC-Presets. Schlüssel: 'raum-heizung' | 'vorlauf' | 'kaelte'
   *  | 'druck' | 'feuchte' | 'pid-demo' */
  loadPreset(name = 'raum-heizung') {
    const P = {
      'raum-heizung': { mode:'heizen',  unit:'°C',  xp:3,  tn:600, tv:0,  sp:21, ks:0.25, tt:20, t1:300, y0:15 },
      'vorlauf':      { mode:'heizen',  unit:'°C',  xp:8,  tn:300, tv:0,  sp:55, ks:0.70, tt:8,  t1:90,  y0:35 },
      'kaelte':       { mode:'kuehlen', unit:'°C',  xp:20, tn:420, tv:0,  sp:19, ks:0.30, tt:15, t1:180, y0:25 },
      'druck':        { mode:'heizen',  unit:'Pa',  xp:10, tn:120, tv:0,  sp:50, ks:0.80, tt:2,  t1:15,  y0:20 },
      'feuchte':      { mode:'heizen',  unit:'%rH', xp:15, tn:600, tv:0,  sp:45, ks:0.50, tt:20, t1:240, y0:30 },
      'pid-demo':     { mode:'heizen',  unit:'°C',  xp:5,  tn:180, tv:30, sp:22, ks:0.40, tt:8,  t1:90,  y0:15 },
    }[name];
    if (!P) return;
    this.mode = P.mode; this.unit = P.unit;
    this.xp = P.xp; this.tn = P.tn; this.tv = P.tv;
    this.sp = P.sp; this.ks = P.ks; this.tt = P.tt; this.t1 = P.t1;
    this.reset(P.y0);
  }
  ...
  #commitDisplay() {
    ...
    this.display = {
      ...
      t: s.t, sp: s.spEff, status, sat,
      dist: s.dist, autoDist: s.autoDistFiltered,
    };
  }
}
